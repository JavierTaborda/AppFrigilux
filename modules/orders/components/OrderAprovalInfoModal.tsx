    ...
    return (
        <BottomModal visible={visible} onClose={onClose} heightPercentage={0.85}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="mb-4">
                ...
                <Row label="Cliente" value={`${order.co_cli.trim()} - ${order.cli_des.trim()}`} multiline />
                ...
                <View className="flex-row gap-2 mb-4 justify-center">
                    <View className={`flex-1 ${isRevisado ? 'bg-green-100 dark:bg-green-800' : 'bg-warning dark:bg-yellow-600'} rounded-lg p-4`}>
                        ...
                        <View className="flex-row items-center gap-2">
                            ...
                            />
                            <Text className={`text-2xl font-semibold ${isRevisado ? 'text-green-700 dark:text-green-300' : 'text-yellow-700 dark:text-yellow-300'}`}>
                ...
                <View className="flex-row gap-2 mb-4">
                    <View className={`flex-1 ${isAuth ? 'bg-tertiary dark:bg-dark-tertiary' : 'bg-warning dark:bg-yellow-600'} rounded-lg p-4`}>
                        {/* <Text className="text-sm text-gray-500 dark:text-gray-400 mb-1 font-medium">
                            Condición
                        </Text> */}
                        <Text className={`text-2xl font-semibold ${isAuth ? 'text-blue-950 dark:text-dark-foreground' : 'text-yellow-700 dark:text-yellow-300'}`}>
