...
export function useGoalsResumen(searchText: string) {
    ...
    const resumen = useMemo(() => {
        const totalAsignada = 1250//goals.reduce((sum, g) => sum + (g.asignado || 0), 0) || 0;
        const totalUtilizado = goals.reduce((sum, g) => sum + (g.utilizado || 0), 0) || 0;
    ...
    return {
        goals, loadGoals, loading, error, ...resumen,     
        // refresh
        handleRefresh,
        refreshing,
        canRefresh,
